...
直播间（`live.bilibili.com`）同样支持自动优选与卡顿切换。直播候选只来自当前直播流实际签发的同族节点：点播的 UPOS 镜像种子无法服务直播路径（实测 403），所以测速前扩展会重放一次播放器自己的播放接口请求，收集 B 站轮换签发的其他集群作为额外候选。优选结果只对当前一路流有效，重连或切换清晰度后会自动重测。

高延迟链路上如果仍频繁"转圈"，可以在直播间弹窗里打开"FLV 优先"：1 秒小分段的 HLS 每段都要付一次网络往返开销，跨境链路上极易临界卡顿；FLV 长连接建连一次后持续接流，实测同链路稳定得多。切换后播放器会自动重连，延迟量级不变（都是几秒），弹幕不受影响。
...
复测只会在可见的视频页正在播放、确实有媒体请求且缓冲充足时发生。只开着首页、动态页或暂停的视频，不会在后台偷偷测速。直播间不使用这套定时复测：每路流只在开播进入时测一次，流变化时重测。
